...
public class AuthenticationPlainCommand extends AbstractAuthenticationCommand {
    ...
    public ByteBuf getCommandLineBytes() throws SmtpAsyncClientException {
        ...
        // NULL separates the fields of the payload below, so a control character in any argument would inject an extra field
        if (authorizationIdentity != null) {
            authzidBuilder.append(validateArgument(authorizationIdentity, "authorization identity"));
        }
        final byte[] secret = Base64.encodeBase64(authzidBuilder.append(SmtpClientConstants.NULL).append(validateArgument(username, "username"))
                .append(SmtpClientConstants.NULL).append(validateArgument(password, "password")).toString().getBytes(StandardCharsets.US_ASCII));
